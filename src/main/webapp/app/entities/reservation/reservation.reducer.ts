...
export const ACTION_TYPES = {
  SEARCH_RESERVATIONS: 'reservation/SEARCH_RESERVATIONS',
...
export default (state: ReservationState = initialState, action): ReservationState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_RESERVATIONS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_RESERVATIONS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_RESERVATIONS):
...
const apiSearchUrl = 'api/_search/reservations';

// Actions

export const getSearchEntities: ICrudSearchAction<IReservation> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_RESERVATIONS,
  payload: axios.get<IReservation>(`${apiSearchUrl}?query=${query}`)
});

export const getEntities: ICrudGetAllAction<IReservation> = (page, size, sort) => ({
  type: ACTION_TYPES.FETCH_RESERVATION_LIST,
  payload: axios.get<IReservation>(`${apiUrl}?cacheBuster=${new Date().getTime()}`)
});
